...
export default function Sidebar({ 
  ...
  const [apiErrors, setApiErrors] = useState<Record<string, { status?: number, message: string }>>({});
    ...
    try {
      ...
      const payload = {
        ...
        courses: courses.map(c => ({
          name: c.name,
          schedule: `${formatDateWithDay(c.startDate)} ~ ${formatDateWithDay(c.endDate)}`,
          // 내용마다 앞에 글머리표(•)를 붙여서 병합
          contents: c.contents
  ...
  const handleExportHWP = () => {
    let html = `
      ...
      <head>
        <meta charset="utf-8">
        <title>${basicInfo.month}월 교육과정 안내</title>
        <style>
          table { width: 100%; border-collapse: collapse; margin-top: 20px; font-family: 'Malgun Gothic', sans-serif; }
          th, td { border: 1px solid black; padding: 10px; font-size: 11pt; }
          th { background-color: #f1f5f9; text-align: center; font-weight: bold; }
          .center { text-align: center; }
          .content-item { margin-bottom: 4px; }
        </style>
      </head>
      <body>
        <h2 style="text-align: center; font-family: 'Malgun Gothic', sans-serif;">
          ${basicInfo.month}월 ${basicInfo.time.includes('오전') ? '오전반' : '오후반'} 교육과정 안내
        </h2>
        <table>
          <thead>
            <tr>
    ...
    courses.forEach(c => {
      const schedule = `${formatDateWithDay(c.startDate)} ~ ${formatDateWithDay(c.endDate)}`;
      ...
      html += `
        <tr>
          <td class="center" style="font-weight: bold;">${c.name}</td>
    ...
    const blob = new Blob([html], { type: 'application/vnd.hancom.hwp;charset=utf-8' });
    const link = document.createElement('a');
    link.href = URL.createObjectURL(blob);
    link.download = `${basicInfo.year}년_${basicInfo.month}월_교육일정.hwp`;
    link.click();
  ...
  const recommendContent = async (courseId: string, courseName: string) => {
    if (isGenerating) {
      alert("현재 다른 항목을 추천 중입니다. 한 번에 1개씩만 처리할 수 있습니다. 완료 후 다시 시도해주세요.");
      return;
    }
    ...
    try {
      ...
      try {
        ...
        if (!response.ok) {
          ...
          throw { status: response.status, message: errorData?.error?.message || 'API 호출에 실패했습니다.' };
        }
        ...
        try {
          ...
          throw new Error('JSON 파싱 에러: ' + (e as Error).message);
        }
      } catch (error: any) {
        ...
        // 2. 내용 강제 업데이트 (stale closure 방지)
        setCourses(prev => prev.map(c => c.id === courseId ? { ...c, contents: ['[오류] API 호출에 실패했습니다. 아래 메시지를 확인하세요.'] } : c));
      }
    } else if (openApiKey) {
      try {
        updateCourse(courseId, { contents: ['AI가 추천 중입니다... 잠시만 기다려주세요.'] });
    ...
    <div className="sidebar">
          ...
          <div key={course.id} className="course-card">
            ...
            <div className="form-group">
              ...
              <div style={{ display: 'flex', gap: '8px' }}>
                ...
                <button 
                  className={`btn ${isGenerating ? 'btn-outline' : 'btn-primary'}`} 
                  style={{ padding: '8px', flexShrink: 0, opacity: isGenerating ? 0.5 : 1 }} 
              ...
              {apiErrors[course.id] && (
                <div style={{ marginTop: '8px', padding: '8px', backgroundColor: '#fee2e2', color: '#b91c1c', borderRadius: '4px', fontSize: '0.85rem' }}>
                  <strong>API 오류 발생 (상태 코드: {apiErrors[course.id].status})</strong><br/>
                  {apiErrors[course.id].message}
                </div>
              )}
            </div>
      ...
      <div>
        ...
        <button 
          ...
          style={{ width: '100%', marginTop: '8px', padding: '12px', fontWeight: 'bold' }} 
